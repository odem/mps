...
function M.lspkeys(ev)
	...
	K.map("n", "gf", vim.lsp.buf.format, opt(ev, "LSP [f]ormat"))
	...
	K.map("n", "K", vim.lsp.buf.signature_help, opt(ev, "LSP [s]ignature [h]elp"))
	K.map("n", "k", vim.lsp.buf.hover, opt(ev, "LSP [h]over"))
